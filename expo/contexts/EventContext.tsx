...
export const [EventProvider, useEvents] = createContextHook(() => {
  ...
  const addEvent = useCallback(async (event: Event) => {
    try {
      ...
      // Remote push notifications so recipients see the new event on their
      // home screen even when the app is closed. The backend push service
      // can't access Firebase (permission_denied), so the creating client
      // (which is authenticated) sends pushes via the backend tRPC proxy.
      pushToGroupMembers({
        groupId: event.groupId || '',
      ...
      }).catch((e) => console.warn('[Event] Remote push failed:', e));
